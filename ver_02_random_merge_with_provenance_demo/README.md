...
> **⚠️ Before you deploy this or redistribute anything it produces, read [CAVEAT.md](CAVEAT.md).**
> Any tool that downloads and recombines model weights can, in principle, be misused to obscure a
> maliciously modified model behind an "innocent merge." CAVEAT.md spells out exactly what this
> tool can and can't do on that front, walks through realistic misuse scenarios, and covers the
> concrete safeguards already built in (safetensors-only I/O with no code execution, provenance
> embedded directly in every merged file's metadata, `huggingface.co`-only network access) versus
> what's still on you if you deploy or extend this (auth, rate limiting, and — always — evaluating
> any merged model's actual behavior before trusting or deploying it).

---

## Table of contents

- [⚠️ Misuse potential and safeguards](CAVEAT.md)
...
By default both services are bound to `127.0.0.1` only (not reachable from other machines on your
network) - see [CAVEAT.md](CAVEAT.md) for why. To deliberately expose it more broadly (e.g. your
LAN), and only after putting your own auth in front of it:

```bash
BACKEND_BIND=0.0.0.0 FRONTEND_BIND=0.0.0.0 ./scripts/up.sh
```

The backend also caps how many downloads/merges can run at once (`MAX_CONCURRENT_DOWNLOADS`,
default 4; `MAX_CONCURRENT_MERGES`, default 2) as a basic abuse safeguard; set either to `0` for
unlimited, or override in `docker-compose.yml`'s backend `environment:` block.
...
| File                  | What it is                                                                                     |
|-----------------------|--------------------------------------------------------------------------------------------------|
| `merged.safetensors`  | The actual spliced checkpoint — every shared tensor is either model A's or model B's original bytes, verbatim (never averaged/interpolated), with correctly recomputed header offsets. **Its own `__metadata__` header also embeds full provenance** — both parent repo ids, the swap ratio, seed, a timestamp, and a plain-language disclaimer — so that information travels with the file itself, not just in a separate log (see [CAVEAT.md](CAVEAT.md) for why this matters). |
...
You can always confirm exactly what went into a given `merged.safetensors` file — even one someone
else handed you — by reading its embedded metadata directly (no trust in a separate README or log
required):
...
See **[CAVEAT.md](CAVEAT.md)** for the full picture on why this matters: realistic ways a
weight-merging tool like this could be misused (e.g. to obscure a maliciously modified model
behind an "innocent merge"), what's architecturally ruled out here (no code execution via the
file format, no arbitrary network egress), and what's still your responsibility if you deploy or
extend this beyond local, trusted-network use.
